...
class AdminScoreR2Controller extends Controller
{
    ...
    public function entryWise()
    {
        ...
        foreach ($brands as $b) {
            $b->average = round( BrandJudge::whereBrandId($b->id)
                ->whereRound(2)->pluck('total')->average(),2);
        }
    ...
    public function judgeWiseEntries(Judge $judge)
    {
        ...
        foreach ($brands as $b) {
            $names[] = $b->name;
            $scores[] = BrandJudge::whereBrandId($b->id)->whereJudgeId($judge->id)->whereRound(2)
                ->pluck('total')[0];
        }
        ...
        foreach ($csrBrands as $b) {
            $names[] = $b->name;
            $scores[] = CsrScore::whereBrandId($b->id)->whereJudgeId($judge->id)->whereRound(2)
                ->pluck('total')[0];
        }
    ...
    public function entryWiseJudges(Brand $brand)
    {
        ...
        if($brand->category->code == 'CSR') {
            ...
            foreach ($judges as $j) {
                $names[] = $j->name;
                $scores[] = CsrScore::whereBrandId($brand->id)->whereJudgeId($j->id)->whereRound(2)
                    ->pluck('total')[0];
            }
            ...
            foreach ($judges as $j) {
                $names[] = $j->name;
                $scores[] = BrandJudge::whereBrandId($brand->id)->whereJudgeId($j->id)->whereRound(2)
                    ->pluck('total')[0];
            }
        }
